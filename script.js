...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function isShopItemOwned(item) {
    try {
      ...
      if (item.category === "passes") {
        ...
        if (item.id === "twice-weight-pass") return !!playerState.hasWeightPass;
      }
  ...
  function triggerRebirth() {
    if (playerState.level < 50) {
      ...
      showNotification(
        "Locked",
        "You must reach Level 100 to Rebirth!",
        "error",
      );
      return;
    }
    ...
    openDetailModal(
      ...
      ],
      {
        ...
        callback: () => {
          ...
          showNotification(
            "ASCENDED",
            `Welcome to Rebirth Rank ${playerState.rebirthCount}!`,
            "level-up",
            6000,
          );
  ...
  function updateStatsUI() {
    ...
    if (labelEnergy) {
      ...
      labelEnergy.textContent =
        energyVal > 9999 ? `${formatMoney(energyVal, true)}%` : `${energyVal}%`;
    }
  ...
  function renderInventoryTray() {
        ...
        if (playerState.inventory.length === 0) {
            inventoryScroll.innerHTML = `<div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 50px 0;">Bag is Empty</div>`;
            return;
        }
        ...
        playerState.inventory.forEach((item, index) => {
            ...
            card.addEventListener('click', () => {
                SoundEngine.playClick();
                if (item.isNew) { item.isNew = false; saveGame(); renderInventoryTray(); }
    
                let sourceCave = "Unknown Caverns";
                const foundCave = cavesData.find(c => c.lootPool && c.lootPool.some(o => o.name === item.name));
                if (foundCave) sourceCave = foundCave.name;
    
                const rarityLabels = { common: "⚪ Common", uncommon: "🟢 Uncommon", rare: "🔵 Rare", epic: "🟣 Epic", legendary: "🟡 Legendary", mythic: "🔴 Mythic", divine: "✨ Divine", cosmic: "🌌 Cosmic" };
    
                let description = `A pristine specimen of ${item.name}.`;
                if (item.mutation && item.mutation !== 'Normal') {
                    const foundMutation = mutationsData.find(m => m.name === item.mutation);
                    if (foundMutation) description = foundMutation.desc;
                }
    
                const itemPrice = Math.floor((item.finalValue || item.baseValue) * playerState.sellMultiplier * playerState.rebirthMultiplier);
      ...
      playerState.inventory.forEach((item) => {
        ...
        const itemPrice = Math.floor(
          (item.finalValue || item.baseValue) *
            ...
            playerState.rebirthMultiplier, // <--- ADD THIS
        );
  ...
  function mineCave(caveId, clickEvent) {
    ...
    // Auto-scroll the grid to the bottom to see the newest item
    const grid = document.getElementById('inventoryScroll');
    if (grid) {
        grid.scrollTo({
  ...
  function rollBonusWeight(clickEvent) {
    if (playerState.hasWeightPass) {
      ...
      if (roll <= 0.5) {
        ...
        spawnFloatingText(
          `+${getBonusWeight} Bonus Weight`,
          clickEvent.clientX,
          clickEvent.clientY + 30,
          "float-ore",
        );
    ...
    try {
      ...
      filteredItems.forEach((item) => {
        try {
          ...
          const isNewItem =
            item.releaseDate &&
            Date.now() - Date.parse(item.releaseDate) < 3 * 24 * 60 * 60 * 1000;
  ...
  function openShopItemDetailModal(itemId) {
    ...
    const stats = [
      {
        label: "Item Category",
        value: categoryLabels[item.category] || item.category.toUpperCase(),
      },
      {
        label: "Upgrade Cost",
            ...
            .forEach((pass) => {
              try {
                ...
                const isBought =
                  ...
                  (pass.id === "twice-weight-pass" &&
                    playerState.hasWeightPass);
                ...
                cave.lootPool.forEach((ore) => {
                  try {
                    ...
                    tiers.forEach((tier) => {
                      const isObtained =
                        playerState.unlockedOres &&
                        playerState.unlockedOres[tier.id] === true;
                      const card = document.createElement("div");
                      card.className = "col-card";
                      card.innerHTML = `
  ...
  if (dailyClaimBtn) {
    dailyClaimBtn.addEventListener("click", (e) => {
      ...
      if (timePassed < cooldown) {
        ...
        const goldReward =
          500 + Math.floor(Math.random() * 1001) + playerState.level * 50;
  ...
  function renderLeaderboard() {
    ...
    const statLabel = leaderCatSelect.options[
      leaderCatSelect.selectedIndex
    ].text.replace("By ", "");
    ...
    db.ref("leaderboards")
      ...
      .once("value", (snapshot) => {
        ...
        if (players.length === 0) {
          leaderboardBody.innerHTML =
            '<tr><td colspan="3" style="text-align:center; padding:20px;">No players ranked yet. Mine or Level up to join!</td></tr>';
          return;
        }
  ...
  window.devLevelUp = () => {
    ...
    playerState.xpNeeded = Math.floor(
      playerState.level * (playerState.xpNeeded * 1.2),
    );
